...
Texture2D _blurShadowTex:register(t11);//ぼかしをかけたライトからの深度値
...
float CalcVSWeight(float2 shadowUV, float ld)
{
	...
	float4 d = _shadowTex.Sample(_samplerState, shadowUV);
	if (shadowUV.x == satUV.x&&shadowUV.y == satUV.y&&d.r < ld - 0.01f
